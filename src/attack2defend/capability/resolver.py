...
class CapabilityResolver:
    ...
    def _coverage_defense_ids(self, related_ids: set[str]) -> set[str]:
        ...
        for node_id in related_ids:
            ...
            for key in ("controls", "detections", "evidence"):
    ...
    def _controls_for_d3fend(self, d3fend_id: str) -> list[str]:
        ...
        for edge in self.outgoing.get(d3fend_id, []):
            if edge.get("relationship") == "implemented_by":
                target = normalize_identifier(edge.get("target", ""))
...
def allowed_transition(source_type: str, target_type: str, relationship: str) -> bool:
    return (
        (relationship in {"has_weakness", "has_related_weakness"} and source_type == "cve" and target_type == "cwe")
        or (relationship == "may_enable_attack_pattern" and source_type == "cwe" and target_type == "capec")
        or (relationship == "child_of" and source_type == "capec" and target_type == "capec")
        or (relationship == "may_map_to_attack_technique" and source_type == "capec" and target_type == "attack")
        or (relationship in {"may_be_defended_by", "may_be_detected_by"} and target_type == "d3fend")
        or (relationship in {"may_lead_to_post_exploitation", "subtechnique_of"} and source_type == "attack" and target_type == "attack")
        or (relationship == "implemented_by" and source_type == "d3fend" and target_type == "control")
        or (relationship == "enables_detection" and source_type == "control" and target_type == "detection")
