...
function NodeDetail({ node, relatedEdges, onSelect, nodeMap, bundle }: { node: RouteNode; relatedEdges: RouteEdge[]; onSelect: (id: string) => void; nodeMap: Map<string, RouteNode>; bundle: KnowledgeBundle }) {
  const semantic = findSemanticRoute(bundle, node.id);
  return (
    <aside className="panel">
      ...
      {semantic && (
        <div>
          <h4>Semantic route status</h4>
          <p>coverage_status: <strong>{semantic.coverage_status ?? 'unknown'}</strong></p>
          <p>confidence_score: <strong>{String(semantic.confidence_score ?? 'unknown')}</strong></p>
          <p>missing_segments: {(semantic.missing_segments ?? []).join(', ') || 'none'}</p>
        </div>
      )}
          ...
          return (
            <li key={`${edge.source}-${edge.relationship}-${edge.target}`}>
              <button className="edge-button" onClick={() => onSelect(otherId)}>
                ...
                <small>{edge.source_kind ?? 'unknown source'} · {edge.source_ref ?? 'source_ref n/a'} · {edge.confidence ?? 'unknown confidence'} · {edge.owner ?? 'unowned'} · {edge.priority ?? 'normal'}</small>
              </button>
            </li>
          );
        })}
...
function resolveRoute(bundle: KnowledgeBundle, roots: string[]): ResolvedRoute {
  ...
  while (queue.length) {
    ...
    const pivot = getPivotEdges(bundle, current.id);
    const edges = pivot.length ? pivot : bundle.edges.filter((edge) => edge.source === current.id || edge.target === current.id);
...
function buildSuggestions(bundle: KnowledgeBundle, query: string) {
  ...
  if (!term) return [];
  const indexEntries = bundle.indexes?.search ?? [];
  if (indexEntries.length) {
    const nodeMap = new Map(bundle.nodes.map((node) => [node.id, node]));
    return indexEntries
      .filter((entry) => entry.text.includes(term) || entry.id.toLowerCase().includes(term) || entry.name.toLowerCase().includes(term))
      .map((entry) => nodeMap.get(entry.id))
      .filter(Boolean)
      .slice(0, 8) as RouteNode[];
  }
...
function findSemanticRoute(bundle: KnowledgeBundle, inputId: string){
  const upper = inputId.toUpperCase();
  return (bundle.semantic_routes ?? []).find((item) => (item.input ?? '').toUpperCase() === upper || (item.route_id ?? '').toUpperCase().includes(upper));
}
function getPivotEdges(bundle: KnowledgeBundle, id: string): RouteEdge[] {
  const mapEdges = (from: string, to: string, rel: string) => ({ source: from, target: to, relationship: rel });
  const out = bundle.indexes?.outgoing?.[id]?.map((item) => mapEdges(id, item.target, item.relationship)) ?? [];
  const incoming = bundle.indexes?.incoming?.[id]?.map((item) => mapEdges(item.source, id, item.relationship)) ?? [];
  const forwardEdges: RouteEdge[] = [];
  const reverseEdges: RouteEdge[] = [];
  for (const [name, mapping] of Object.entries(bundle.indexes?.forward ?? {})) {
    for (const target of mapping[id] ?? []) forwardEdges.push(mapEdges(id, target, name));
  }
  for (const [name, mapping] of Object.entries(bundle.indexes?.reverse ?? {})) {
    for (const source of mapping[id] ?? []) reverseEdges.push(mapEdges(source, id, name));
  }
  return dedupeEdges([...out, ...incoming, ...forwardEdges, ...reverseEdges]);
}
