...
The AI Route Analyst is available as an offline, validator-gated artifact generated by `scripts/intelligence/curate_route.py --llm`. The UI will render that narrative only when the artifact matches the currently selected input; otherwise it falls back to the deterministic route view.
